...
return (
  ...
  <div>
    <p>
      This release with id: {releaseId} is of version {appRelease?.version}
    </p>
  </div>
);
```
...
return (
  ...
  <div>
    <p>
      The application {extension?.displayName} has {extension?.releasesCount} releases
    </p>
  </div>
);
```
...
const BeamList = () => {
  ...
  return (
    <div>
      ...
      <button
        onClick={() =>
          fetchMore({ variables: { after: data.akashaBeamIndex?.pageInfo.endCursor } })
        }
      >
        Load More
      </button>
...
const MyComponent = () => {
  ...
  return <div>{contentBlock.author.id}</div>;
};
```
  ...
  return (
    <div>
      {data?.node?.akashaProfile?.following?.edges.map(edge => (
        <div key={edge.node?.did?.akashaProfile?.id}>{edge.node?.did?.akashaProfile?.name}</div>
      ))}
  ...
  return (
    <div>
      {data?.node?.akashaProfile?.interests?.map(interest => (
        <div key={interest.id}>{interest.label}</div>
      ))}
    </div>
  );
};
