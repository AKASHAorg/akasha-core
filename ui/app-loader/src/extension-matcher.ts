...
export const extensionMatcher =
  (
    uiEvents: ReplaySubject<UIEventData>,
    props: Omit<RootExtensionProps, 'uiEvents' | 'extensionData' | 'domElement'>,
  ) =>
  (
    extConfig: Record<
      string,
      Record<'load' | 'unload', (props?: RootExtensionProps | UIEventData) => void>
    >,
  ) => {
    uiEvents
      .pipe(
