...
const useAnalytics = (): [UseAnalyticsState, UseAnalyticsActions] => {
  ...
  const actions: UseAnalyticsActions = {
    ...
    optUserOut() {
      if (hookGlobal._paq) {
        hookGlobal._paq.push(['optUserOut']);
      }
      if (globalThis.localStorage) {
        localStorage.setItem(COOKIE_CONSENT_NAME, CookieConsentTypes.ESSENTIAL);
        setState({ cookieBannerDismissed: true, consentType: CookieConsentTypes.ESSENTIAL });
      }
    },
    optUserIn() {
      if (hookGlobal._paq) {
        hookGlobal._paq.push(['forgetUserOptOut']);
      }
      if (globalThis.localStorage) {
        localStorage.setItem(COOKIE_CONSENT_NAME, CookieConsentTypes.ALL);
        setState({ cookieBannerDismissed: true, consentType: CookieConsentTypes.ALL });
      }
    },
    acceptConsent(type) {
      if (globalThis.localStorage) {
