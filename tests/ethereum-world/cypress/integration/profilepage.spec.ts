...
const profileIdToVisit = 'did:pkh:eip155:5:0x36c703c4d2fa2437dc883e2e0884e57404e16493';

describe('Profile Page', () => {
  context('Profile Page', () => {
    before(() => {
      cy.visit(`/@akashaorg/app-profile/${profileIdToVisit}`, { timeout: TIMEOUT });
    });
    ...
    it('should redirect to feed profile page', () => {
      cy.visit(`/@akashaorg/app-profile/${profileIdToVisit}`);
      cy.get('[data-testid="posts-button"]', { timeout: TIMEOUT }).first().click();
      cy.location('pathname').should('contain', `/profile-feed/${profileIdToVisit}`);
    });
